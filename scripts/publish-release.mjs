...
async function buildPublicationArtifacts({ plan, artifactsDir, outputDir }) {
  ...
  const mergedInventory = {
    ...
    canonicalVersionInput: plan.release.canonicalVersionInput,
    windowsStoreVersion: plan.release.windowsStoreVersion,
...
function buildReleaseBody({ plan, publicationArtifacts, publishedAt, githubReleaseAssets }) {
  ...
  return [
    ...
    `- Store package assets: ${publicationArtifacts.mergedInventory.artifacts.filter((artifact) => /\.msix$/i.test(artifact.fileName)).length}`,
